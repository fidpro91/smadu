...
<div class="col-md-12">
	<?= form_open("absensi_siswa/save", ["method" => "post", "class" => "form-horizontal", "id" => "fm_absensi_siswa","enctype"=>"multipart/form-data"], $model) ?>
		<?= form_hidden("absen_id") ?>
		<?= form_hidden("absen_code") ?>
		<?= form_hidden("siswa_id") ?>
	...
	<div class="row panel-footer" align="center">
		<button class="btn btn-primary" type="button" onclick="$('#fm_absensi_siswa').submit()">Save</button>
		<button class="btn btn-warning" type="button" id="btn-cancel">Cancel</button>
	</div>
</div>
<script type="text/javascript">
	$("#btn-cancel").click(() => {
		$("#form_absensi_siswa").hide();
		$("#form_absensi_siswa").html('');
	});
	$("body").on("focus", "#st_name", function() {
		$(this).autocomplete({
			source: "<?php echo site_url('ms_siswa/get_siswa'); ?>",
			select: function(event, ui) {
				$('#siswa_id').val(ui.item.st_id);
				$('#absen_code').val(ui.item.finger_id);
			}
		});
	});
