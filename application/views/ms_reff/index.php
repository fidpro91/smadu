...
<div class="panel panel-info">
  <div class="panel-heading">
    <h3 class="panel-title">Form Ms Reff</h3>
    <div class="panel-options">
      <button type="button" id="btn-add" class="btn btn-black">
        <i class="entypo-plus"></i> Add</button>
    </div>
  </div>
  <div class="panel-body" id="form_ms_reff" style="display: none;">
  </div>
  <div class="panel-body" id="data_ms_reff">
    <?=create_table("tb_ms_reff","M_ms_reff",["class"=>"table table-bordered datatable" ,"style" => "width:100% !important;"])?>
  </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    ...
    $('#tb_ms_reff').closest('.dataTables_wrapper').find('select').select2({
  ...
  $("#checkAll").click(() => {
    if ($("#checkAll").is(':checked')) {
      $("#tb_ms_reff input[type='checkbox']").attr("checked", true);
    } else {
      $("#tb_ms_reff input[type='checkbox']").attr("checked", false);
    }
  });

  $("#btn-deleteChecked").click(function (event) {
    event.preventDefault();
    var searchIDs = $("#tb_ms_reff input:checkbox:checked").map(function () {
      return $(this).val();
    }).toArray();
    ...
    if (confirm("Anda yakin akan menghapus data ini?")) {
      $.post('ms_reff/delete_multi', { data: searchIDs }, (resp) => {
        if (resp.code == '200') {
