...
class Builder extends MY_Generator {
	...
	public function index()
	{
		...
		$mydir = 'application/core';
		$data['coreSistem'] = array_diff(scandir($mydir), array('.', '..'));
		$menu = [];
	...
	public function create_mvc()
	{
		...
		$coreSistem = $this->input->post('core_sistem');
		$data = $this->m_builder->get_fieldTable($tableName);
		if ($this->input->post('c_controller')) {
			$this->_createController($tableName,$data,$coreSistem);
		}
	...
	public function _createController($tableName,$data,$coreSistem)
	{
		$className = $tableName;
		...
		$file = file_get_contents(APPPATH."docs/mvc/controller.php");
		$file = str_replace('$classNameHeader', ucfirst($className), $file);
		$file = str_replace('$className', ($className), $file);
		$file = str_replace('$coreSistem', ($coreSistem), $file);
		$file = str_replace('$tableName', strtolower($tableName), $file);
	...
	public function _createModel($tableName,$data)
	{
		...
		$file = file_get_contents(APPPATH."docs/mvc/model.php");
		$column = "[\n";
		$rule = "";
		$pkey = null;
		foreach ($data as $key => $value) {
	...
	public function _createView($tableName,$data)
	{
		...
		$txt = file_get_contents(APPPATH."docs/mvc/views/form.html");
		$txt2 = file_get_contents(APPPATH."docs/mvc/views/index.html");
		$form = "\t\t\t".'<?=form_open("'.$className.'/save",["method"=>"post","class"=>"form-horizontal","id"=>"fm_'.$className.'"],$model)?>'."\n";
		$hd=[];
		foreach ($data as $key => $value) {
			if ($value->primary_key > 0) {
		...
		$hd = implode(",\n",$hd);
		$headerTable = '<?=create_table("tb_'.$className.'","M_'.$className.'",["class"=>"table table-bordered datatable" ,"style" => "width:100% !important;"])?>';
		$form .= '<?=form_close()?>';
